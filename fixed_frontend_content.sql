...
CREATE POLICY "Admins can view DSS applications" ON public.dss_applications FOR SELECT USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid())::uuid AND ur.role = 'admin'));
CREATE POLICY "Admins can update DSS applications" ON public.dss_applications FOR UPDATE USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = (SELECT auth.uid())::uuid AND ur.role = 'admin'));
