-- =========================================
-- COMPLETE AFRIKSPARK DATABASE SETUP WITH ALL CONTENT
-- This SQL creates ALL tables and populates with content for frontend display
-- =========================================
...
CREATE POLICY "Admins can manage files" ON storage.objects FOR ALL USING (bucket_id IN ('project-logos', 'blog-images', 'testimonials', 'general') AND EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = auth.uid() AND role = 'admin'));
...
-- ✅ Projects page with 3 featured projects
